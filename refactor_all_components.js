...
files.forEach(file => {
  ...
  if (!html.includes('class="layout') && !html.includes("class='layout") && !html.includes('class="layout ')) {
    ...
    html = html.replace(/<h[23][^>]*>(.*?)<\/h[23]>\s*(<p[^>]*>.*?<\/p>)?/i, (match, title, desc) => {
      ...
      if (title.toLowerCase().includes('calculator') || title.toLowerCase().includes('pay') || title.toLowerCase().includes('tax') || title.toLowerCase().includes('ssp') || title.toLowerCase().includes('smp')) {
        return '';
      }
      return match;
    });
...
`;
    ...
    // (If html already had <slot />, we only keep one inside main-col)
    const slotCount = (html.match(/<slot\s*\/?>/g) || []).length;
    if (slotCount > 1) {
...
console.log(`Refactored ${updatedCount} components to standard 2-column sidebar layout.`);
