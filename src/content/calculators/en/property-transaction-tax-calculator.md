---
title: "Property Transaction Tax Calculator UK — SDLT, LBTT & LTT"
description: "Calculate UK property purchase taxes including Stamp Duty Land Tax (SDLT), Land and Buildings Transaction Tax (LBTT), and Land Transaction Tax (LTT)."
keywords: "property transaction tax calculator uk sdlt, lbtt & ltt, calculate uk property purchase taxes including, hmrc 2026/27"
category: "impuestos"
lang: "en"
isDevolved: true
translationUrl: "/property-transaction-tax-calculator/"
labels:
  inputTitle: "Property Purchase Details"
  propertyValueLabel: "Property Purchase Price (£)"
  buyerTypeLabel: "Buyer Status (First-Time Buyer, Moving Home, Additional Property)"
  nationLabel: "UK Nation Location (England/NI, Scotland, Wales)"
  resultLabel: "Total Property Transaction Tax Due"
  effectiveRateLabel: "Effective Tax Rate (%):"
  breakdownTitle: "Property Tax Band Breakdown"
calcBadgesTitle: "Property Tax Assurances"
taxYearBadge: "2026/27 SDLT Rates"
taxYearBadgeDesc: "Calculates Stamp Duty Land Tax for England & Northern Ireland using statutory progressive threshold bands."
devolBadge: "Devolved LBTT & LTT"
devolBadgeDesc: "Includes Scottish Land and Buildings Transaction Tax (LBTT) and Welsh Land Transaction Tax (LTT)."
taperBadge: "3%/6% Surcharge Math"
taperBadgeDesc: "Integrates additional residential property surcharges (3% for England SDLT; 6% ADS for Scotland LBTT)."
authoritiesTitle: "Reference Authorities"
auth1Name: "HM Revenue & Customs (SDLT)"
auth1Desc: "The UK tax authority regulating Stamp Duty Land Tax in England and Northern Ireland."
auth1Link: "https://www.gov.uk/stamp-duty-land-tax"
auth2Name: "Revenue Scotland (LBTT)"
auth2Desc: "The devolved tax authority administering Land and Buildings Transaction Tax in Scotland."
auth2Link: "https://www.revenue.scot/land-buildings-transaction-tax"
sourcesTitle: "Official Sources"
source1: "Finance Act 2003 (SDLT legislation) →"
lastUpdatedTitle: "Updated for 2026/27:"
lastUpdatedDesc: "Aligned with current UK statutory property tax bands."
---

Buying real estate in the UK involves paying statutory property transaction tax to the government. The tax paid depends on the purchase price, buyer status, and the UK nation where the property is located.

- **England & Northern Ireland**: **Stamp Duty Land Tax (SDLT)**.
- **Scotland**: **Land and Buildings Transaction Tax (LBTT)**.
- **Wales**: **Land Transaction Tax (LTT)**.

## ⚙️ Statutory Property Tax Bands & Surcharges for 2026/27

### 1. SDLT Bands (England & Northern Ireland — Next Home Buyer)
- **Up to £250,000**: **0%**.
- **£250,001 to £925,000**: **5%**.
- **£925,001 to £1,500,000**: **10%**.
- **Over £1,500,000**: **12%**.

### 2. First-Time Buyer Relief (England & NI)
First-time buyers pay **0% SDLT on purchases up to £425,000**, and **5%** on the portion between £425,001 and £625,000. (If property value exceeds £625,000, standard buyer rates apply).

### 3. Additional Property Surcharges (Buy-to-Let / Second Homes)
Purchasing an additional residential property (such as a holiday home or buy-to-let investment) incurs a severe statutory surcharge across all price bands:
- **England & NI**: Extra **3% SDLT surcharge**.
- **Scotland**: Extra **6% Additional Dwelling Supplement (ADS)**.
- **Wales**: Higher residential LTT band rates apply.

---

## 📊 Practical Property Tax Worked Examples

Below are two worked calculation examples illustrating SDLT property taxes in England:

<div class="example-box">
  <strong>Example 1: Buying a next home in England for £350,000 (Standard Rates)</strong>
  <ul>
    <li>Property Price: **£350,000.00**</li>
    <li>First £250,000 @ 0%: **£0.00**</li>
    <li>Remaining £100,000 (£250k to £350k) @ 5%: **£5,000.00**</li>
  </ul>
  <p>Calculation: Total SDLT due = £0.00 + £5,000.00 = £5,000.00. Effective tax rate = 1.43%.</p>
  <div class="net-highlight">Total Stamp Duty Land Tax (SDLT): **£5,000.00** (Effective Rate: **1.43%**)</div>
</div>

<div class="example-box">
  <strong>Example 2: Buying a Buy-to-Let property in England for £350,000 (+3% Surcharge)</strong>
  <ul>
    <li>Property Price: **£350,000.00** (Additional Property)</li>
    <li>First £250,000 @ 3% surcharge: **£7,500.00**</li>
    <li>Remaining £100,000 @ 8% (5% + 3%): **£8,000.00**</li>
  </ul>
  <p>Calculation: Total SDLT due = £7,500.00 + £8,000.00 = £15,500.00. Effective tax rate = 4.43%.</p>
  <div class="net-highlight">Total Stamp Duty Land Tax (SDLT): **£15,500.00** (Effective Rate: **4.43%**)</div>
</div>

---

## 📑 Common Pitfalls & Property Tax Warnings

1. **Failing to Claim Second Home Refund Following Replacement of Main Residence**: If you buy a new main home before selling your old main home, you must pay the 3% additional property surcharge upfront. However, if you sell your old main home within **36 months**, you can claim a **100% refund of the 3% surcharge** from HMRC.
2. **First-Time Buyer Relief Qualification Rules**: Both joint buyers must be genuine first-time buyers who have never owned an interest in any residential property anywhere in the world.
3. **Non-UK Resident Surcharge**: Non-UK residents purchasing residential property in England face an additional **2% non-resident SDLT surcharge** on top of standard rates.

---

## ❓ Frequently Asked Questions (FAQ)

<div class="faq-sec">
  <div class="faq-item">
    <button class="faq-trigger">When must Stamp Duty / LBTT property tax be paid? <span class="icon">▼</span></button>
    <div class="faq-content">
      <p>In England & NI, SDLT must be filed and paid to HMRC within **14 days** of completion. In Scotland, LBTT returns must be submitted to Revenue Scotland within **30 days** of completion. Your conveyancing solicitor usually handles payment on completion day.</p>
    </div>
  </div>

  <div class="faq-item">
    <button class="faq-trigger">Are property transaction taxes different in Scotland and Wales? <span class="icon">▼</span></button>
    <div class="faq-content">
      <p>Yes. Scotland uses Land and Buildings Transaction Tax (LBTT) starting at £145,000. Wales uses Land Transaction Tax (LTT) starting at £225,000. Each nation sets independent threshold bands and rates.</p>
    </div>
  </div>

  <div class="faq-item">
    <button class="faq-trigger">Can Stamp Duty be added to my mortgage loan? <span class="icon">▼</span></button>
    <div class="faq-content">
      <p>Yes, mortgage lenders permit buyers to add stamp duty costs to total mortgage borrowing. However, doing so increases your loan-to-value (LTV) ratio and incurs interest over the mortgage term.</p>
    </div>
  </div>

  <div class="faq-item">
    <button class="faq-trigger">Is stamp duty payable on property inherited through a will? <span class="icon">▼</span></button>
    <div class="faq-content">
      <p>No. Properties inherited through a will or legal estate distribution are completely exempt from Stamp Duty Land Tax, LBTT, and LTT.</p>
    </div>
  </div>
</div>
