---
title: "Statutory Maternity & Paternity Pay Calculator UK 2026/27"
description: "Calculate UK Statutory Maternity Pay (SMP) and Statutory Paternity Pay (SPP) weekly amounts and statutory payment schedules."
keywords: "statutory maternity & paternity pay calculator uk 2026 27, calculate uk statutory maternity pay smp, family benefits 2026/27"
category: "familia"
lang: "en"
isDevolved: false
translationUrl: "/statutory-maternity-paternity-pay-calculator/"
labels:
  inputTitle: "Maternity & Paternity Pay Details"
  weeklySalaryLabel: "Average Weekly Gross Earnings (£)"
  resultLabel: "Total 39-Week Statutory Maternity Pay"
  statutoryRateLabel: "Statutory Weekly Flat Rate:"
  breakdownTitle: "Payment Schedule Breakdown"
calcBadgesTitle: "Statutory Assurances"
taxYearBadge: "2026/27 Statutory Rates"
taxYearBadgeDesc: "Uses the official DWP statutory flat rate of £184.03 per week (or 90% of average weekly earnings if lower)."
devolBadge: "39-Week Coverage"
devolBadgeDesc: "Calculates full 6-week initial 90% rate plus 33-week statutory flat rate period."
taperBadge: "PAYE Compliant"
taperBadgeDesc: "Deductions for Income Tax and Class 1 National Insurance apply to SMP and SPP payments."
authoritiesTitle: "Reference Authorities"
auth1Name: "Department for Work & Pensions (DWP)"
auth1Desc: "The government department administering statutory parental allowances and social security policy."
auth1Link: "https://www.gov.uk/maternity-pay-leave"
auth2Name: "GOV.UK Statutory Paternity Pay Guide"
auth2Desc: "Official guidance detailing eligibility and entitlement rules for working fathers and partners."
auth2Link: "https://www.gov.uk/paternity-pay-leave"
sourcesTitle: "Official Sources"
source1: "Social Security Contributions and Benefits Act 1992 (legislation.gov.uk) →"
lastUpdatedTitle: "Updated for 2026/27:"
lastUpdatedDesc: "Aligned with current DWP statutory maternity and paternity pay rates."
---

Statutory Maternity Pay (SMP) and Statutory Paternity Pay (SPP) are statutory payments paid by employers to eligible employees taking time off work to have a baby or adopt a child.

Understanding your exact financial entitlement during maternity or paternity leave allows families to budget accurately for newborn expenses.

## ⚙️ Statutory Payment Schedules & Rates for 2026/27

### Statutory Maternity Pay (SMP) Structure (39 Weeks)
SMP is paid for up to **39 weeks** divided into two distinct payment phases:
- **First 6 Weeks**: Paid at **90% of your average gross weekly earnings** (with no maximum cap).
- **Next 33 Weeks**: Paid at the statutory flat rate of **£184.03 per week**, or **90% of your average gross weekly earnings** (whichever amount is lower).

### Statutory Paternity Pay (SPP) Structure (2 Weeks)
- **Duration**: Eligible fathers and partners can take up to **2 weeks of paid paternity leave**.
- **Rate**: Paid at **£184.03 per week**, or **90% of average weekly earnings** (whichever is lower).

### Qualification Criteria
To qualify for SMP or SPP, you must:
1. Earn at least the **Lower Earnings Limit (LEL)** of **£123.00 per week** in the 8-week calculation period.
2. Have worked continuously for your employer for at least **26 weeks** continuing into the 15th week before the expected week of childbirth.

---

## 📊 Practical Maternity Pay Worked Examples

Below are two worked examples demonstrating how average weekly earnings affect 39-week payouts:

<div class="example-box">
  <strong>Example 1: Employee earning £500.00 / week (£26,000 / year)</strong>
  <ul>
    <li>Average Weekly Earnings: **£500.00**</li>
    <li>First 6 Weeks (90% rate): £500.00 × 90% = **£450.00 / week** (£2,700.00 total)</li>
    <li>Remaining 33 Weeks (Statutory cap): **£184.03 / week** (£6,072.99 total)</li>
  </ul>
  <p>Calculation: Total SMP entitlement = £2,700.00 + £6,072.99 = **£8,772.99**.</p>
  <div class="net-highlight">Total 39-Week Statutory Maternity Pay: **£8,772.99**</div>
</div>

<div class="example-box">
  <strong>Example 2: Part-time employee earning £180.00 / week</strong>
  <ul>
    <li>Average Weekly Earnings: **£180.00**</li>
    <li>First 6 Weeks (90% rate): £180.00 × 90% = **£162.00 / week** (£972.00 total)</li>
    <li>Remaining 33 Weeks (90% applies as lower than £184.03): **£162.00 / week** (£5,346.00 total)</li>
  </ul>
  <p>Calculation: Total SMP entitlement = £972.00 + £5,346.00 = **£6,318.00**.</p>
  <div class="net-highlight">Total 39-Week Statutory Maternity Pay: **£6,318.00**</div>
</div>

---

## 📑 Common Pitfalls & Tax Warnings

1. **Confusing Gross SMP with Net Take-Home Pay**: SMP is subject to standard Income Tax and Class 1 National Insurance Contributions. Your net bank transfer will be lower than the headline £184.03 weekly figure depending on tax codes.
2. **Assuming Company Maternity Pay Replaces SMP**: Enhanced contractual maternity pay offered by generous employers usually includes statutory SMP rather than paying it on top. Check your employment handbook contract terms.
3. **Missing Notice Deadlines**: You must notify your employer at least 15 weeks before your baby is due, stating your expected due date and when you wish your maternity leave to begin.

---

## ❓ Frequently Asked Questions (FAQ)

<div class="faq-sec">
  <div class="faq-item">
    <button class="faq-trigger">Can I work small hours while receiving Statutory Maternity Pay? <span class="icon">▼</span></button>
    <div class="faq-content">
      <p>Yes. You can work up to 10 'Keep in Touch' (KIT) days during your maternity leave without losing your SMP or bringing your statutory leave period to an end. KIT days must be mutually agreed with your employer.</p>
    </div>
  </div>

  <div class="faq-item">
    <button class="faq-trigger">What happens if I get a salary pay rise while on maternity leave? <span class="icon">▼</span></button>
    <div class="faq-content">
      <p>Under UK employment law (the Alabaster ruling), if your employer awards a pay rise at any time between the start of the calculation period and the end of your maternity leave, your SMP must be recalculated as if the pay rise had applied during the reference period.</p>
    </div>
  </div>

  <div class="faq-item">
    <button class="faq-trigger">How is Statutory Paternity Pay (SPP) paid to fathers? <span class="icon">▼</span></button>
    <div class="faq-content">
      <p>SPP is paid in the same way as your regular salary (weekly or monthly) through payroll. You can take either 1 or 2 consecutive weeks of paternity leave, but it must be taken within 52 weeks of the birth.</p>
    </div>
  </div>

  <div class="faq-item">
    <button class="faq-trigger">What if I do not meet the 26-week qualification period for SMP? <span class="icon">▼</span></button>
    <div class="faq-content">
      <p>If you do not qualify for SMP from your employer, they must give you a form SMP1. You can then apply to the government for **Maternity Allowance (MA)**, which provides up to £184.03 per week for 39 weeks via Jobcentre Plus.</p>
    </div>
  </div>
</div>
